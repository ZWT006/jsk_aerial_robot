...
public:
          ...
          decimation_(decimation)
    {
        ...
        std::string bake_topic = "/beetle_omni/teleop_command";
        ...
        start_sub_          = nh_.subscribe(bake_topic + "/start",          1, &PolicyDeployer::_control_enable_callback, this);
        takeoff_sub_        = nh_.subscribe(bake_topic + "/takeoff",        1, &PolicyDeployer::_control_disable_callback, this);
        force_landing_sub_  = nh_.subscribe(bake_topic + "/force_landing",  1, &PolicyDeployer::_control_disable_callback, this);

        // Publishers
        thrust_pub_ = nh_.advertise<FourAxisCommand>("/beetle_omni/four_axes/command", 1);
        thrust_debug_pub_ = nh_.advertise<FourAxisCommand>("/beetle_omni/four_axes/command_debug", 1);
        gimbal_pub_ = nh_.advertise<sensor_msgs::JointState>("/beetle_omni/gimbals_ctrl", 1);
        gimbal_debug_pub_ = nh_.advertise<sensor_msgs::JointState>("/beetle_omni/gimbals_ctrl_debug", 1);
        obs_debug_pub_ = nh_.advertise<std_msgs::Float32MultiArray>("/beetle_omni/observation_debug", 1);
        // defaults
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("Waiting for transform from beetle-omni/root to beetle-omni/cog...");
    tf2_ros::TransformListener tf2_listener(tf2_buffer);
    geometry_msgs::TransformStamped lookupTransform;
    bool get_initial_transform = false;
    while (ros::ok() && !get_initial_transform) {
        try {
            lookupTransform = tf2_buffer.lookupTransform("beetle_omni/cog","beetle_omni/root",ros::Time(0));
