...
public:
  ...
  inline uint16_t getMotorNumber()
  {
    return motor_number_;
  }

  void setMotorNumber(uint8_t motor_number);
  void setGimbalDof(uint8_t gimbal_dof)
  {
    gimbal_dof_ = gimbal_dof;
  }
  uint16_t getGimbalDof()
  {
    return gimbal_dof_;
  }
  void setPwmTestMode(bool pwm_test_flag)
  {
    pwm_test_flag_ = pwm_test_flag;
  }
  bool getIntegrateFlag()
  {
    return integrate_flag_;
  }
  void setIntegrateFlag(bool integrate_flag)
  {
    integrate_flag_ = integrate_flag;
  }
  bool getForceLandingFlag()
  {
    return force_landing_flag_;
  }
  ...
  float getPwm(uint8_t index)
  {
    return target_pwm_[index];
  }
  float getForce(uint8_t index)
  {
...
private:
  ...
  {
    return ros::Time::now().toSec() * 1000;
  }

public:
  void useGroundTruth(bool flag)
  {
    use_ground_truth_ = flag;
  }
  void setTrueRPY(float r, float p, float y)
  {
    true_angles_.x = r;
    true_angles_.y = p;
    true_angles_.z = y;
