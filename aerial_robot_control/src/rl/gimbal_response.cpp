...
public:
    ...
    void spin() {
        ...
        while (ros::ok()) {
            ...
            for (size_t i = 0; i < num; ++i) {
                ...
                msg.position[i] = last_target_pos_[i] + default_pos;
            }
            ...
            gimbal_data_.back().push_back(static_cast<float>(timestep));
            {
                std::lock_guard<std::mutex> lk(data_mutex_);
                for (size_t i = 0; i < num; ++i) {
                    gimbal_data_.back().push_back(last_gimbal_pos_[i]);
    ...
    std::vector<float> sinTriangleWave(double t, double period, float amplitude) {
        std::vector<float> wave(4, 0.0f);
        double phase = std::fmod(t, period);
        std::vector<float> triangleVal = triangleWave(t, period * 8.0, amplitude);
        amplitude = std::abs(triangleVal[0]);
        double third = period / 3.0;
        // slope to go from -A to +A over one third period: 3*A/period
        double slope = (2.0 * amplitude) / third; // = 3*A/period
        float val;
        if (phase < third) {
            val = static_cast<float>(-amplitude + slope * phase);
        } else if (phase < 2.0 * third) {
            val = static_cast<float>(amplitude - slope * (phase - third));
        } else {
            val = static_cast<float>(-amplitude + slope * (phase - 2.0 * third));
        }
        for (int i = 0; i < 4; ++i) wave[i] = val;
        return wave;
    }
